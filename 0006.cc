...
void* at_free_mm( void* place, dword size, int pid )
{
   tabmm* nlm = NULL,* nbr = NULL,* ntw = NULL,* nlf = NULL;
   void* npg = NULL;
   dword pl = ( dword )( ( byte* )place - memory );
   int ipg = ( pl & 0x0000f000 ) >> 0xc; // starts at 0.
   int ilf = ( pl & 0x000f0000 ) >> 0x10; // starts at 0.
   int itw = ( pl & 0x00f00000 ) >> 20; // starts at 0.
   int ibr = ( pl & 0x0f000000 ) >> 24; // starts at 0.
   int ilm = ( pl & 0xf0000000 ) >> 28; // starts at 0.
   ...
   for(; ilm < 0x10; ++ilm )
   {
      ...
      if( nlm == ( void* )pid )
      {
         ...
         {
            trunk -> m[ ilm ] = NULL;
            return ( byte* )( ( dword )pl + ( dword )memory );
         }
         else return NULL;
      }
      else if( nlm == USED || nlm == LOWM || nlm == KMEM || ( ( dword )nlm < ( dword )0x10000 ) )
         // NULL counts here too.
         return NULL; // not resident; fixme: reallocate or count it up beforehand
      else for(; ibr < 0x10; ++ibr ) // count branches
      {
         nbr = ( tabmm* )( nlm -> m[ ibr ] );
         if( nbr == ( void* )pid )
         {
            if( end > ( ilm << 28 ) + ( ( ibr + 1 ) << 24 ) )
            {
               nlm -> m[ ibr ] = NULL;
               continue;
            }
         ...
         else if( nbr == USED || nbr == LOWM || nbr == KMEM || ( ( dword )nbr < ( dword )0x10000 ) )
            // NULL counts here too.
            return NULL; // not resident; fixme: reallocate or count it up beforehand
         else for(; itw < 0x10; ++itw ) // count branches
         {
            ntw = ( tabmm* )( nbr -> m[ itw ] );
            if( ntw == ( void* )pid )
            {
               //
               if( end > ( ilm << 28 ) + ( ibr << 24 ) + ( ( itw + 1 ) << 20 ) )
               {
                  nbr -> m[ itw ] = NULL;
                  continue;
               }
...
/*
               for(; ilf < 0x10; ++ilf )
               {
                  ...
                  else return NULL; // should not have to build up substructure when freeing,
                  for(; ipg < 0x10; ++ipg )
                  {
                     npg = nlf -> m[ ipg ]; // void*
                     if( npg == pid )
                     {
                        //nlf = ( tabmm* )( ntw -> m[ ilf ] );
                        if( end > ( ( ilm ) << 28 ) + ( ( ibr ) << 24 ) + ( ( itw ) << 20 ) + ( ( ilf ) << 0x10 ) + ( ( ipg + 1 ) << 0xc ) )
                        {
                           nlf -> m[ ipg ] = NULL;
                           continue;
                        }
                        ...
                        {
                           printf( "0003 %08X assertion failed\r\n", end );
                           return NULL;
                        }
                     } // if freeing
                     else return NULL;
...
*/
            ...
            else if( ntw == USED || ntw == LOWM || ntw == KMEM || ( ( dword )ntw < ( dword )0x10000 ) )
               // NULL counts here too.
               return NULL; // not resident; fixme: reallocate or count it up beforehand
            else for(; ilf < 0x10; ++ilf ) // count branches
            {
               nlf = ( tabmm* )( ntw -> m[ ilf ] );
               if( nlf == ( void* )pid )
               {
                  if( end > ( ( ilm ) << 28 ) + ( ( ibr ) << 24 ) + ( ( itw ) << 20 ) + ( ( ilf + 1 ) << 0x10 ) )
                  {
                     nlf -> m[ ipg ] = NULL;
                     continue;
                  }
                  ...
                  {
                     return NULL;
                     //nlf = my_free_mm( klpage );
                     //nlf -> loc = ( ilm << 28 ) + ( ibr << 24 ) + ( itw << 20 ) + ( ilf << 0x10 );
                     //( ( tabmm* )( ntw ) ) -> m[ boom ] = nlf;
                     //leaf_sub( nlf ); // free substructure
                  }
/*
                  for(; ipg < 0x10; ++ipg )
                  {
...
*/
               ...
               else if( nlf == USED || nlf == LOWM || nlf == KMEM || ( ( dword )nlf < ( dword )0x10000 ) )
                  // NULL counts here too.
                  // and if they're all NULL consider cleaning the structure outs
                  return NULL; // not resident; fixme: reallocate or count it up beforehand
               else for(; ipg < 0x10; ++ipg ) // count branches
               {
                  npg = nlf -> m[ ipg ]; // void*
                  if( npg == ( void* )pid )
                  {
...
//--------this space reserved for 0006.cc
//{
//}
//--------
...
void* more( page** param, dword chunk )
{
   ...
   if( chunk > 0x1000 )
   {
      ...
      dword size = 0;
      page* pag = NULL;
      void* mem = kmem_param -> alloc( 0x578 ); // test for enough substructure
      if( mem == NULL )
      {  // size describes the block(s) of substructure found
         void* loc = pages_count_mm( chunk + 0x1000, &size ); // pile on one more
         if( loc != NULL )
         {
            dword pg = ( ( dword )loc + ( dword )chunk - ( dword )memory );
            ...
            pag = ( page* )( pg + ( dword )memory ); // pass the extra page as a parameter to at_alloc_mm
            pag -> clear();
            printf( "extraneous substructure put at: %08x\r\n", pg );
            result = at_alloc_mm( loc, size, pid, pag );
         }
         else printf( "impossible\r\n" ); // here would be a good place to defragmentc
      }
      else // enough kludge memory exists for the allocator to do its work
      {
         //printf( "%d bytes preallocated:\r\n", 0x578 );
         //kmem_param -> dump();
         kmem_param -> free( mem, 0x578 ); // end test for enough substructure
